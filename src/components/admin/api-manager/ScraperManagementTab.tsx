...
export default function ScraperManagementTab() {
  ...
  const testScraper = async (s: ScraperConfig) => {
    ...
    if (result.ok && result.data?.success) {
      ...
      toast({ title: `❌ ${s.carrier} test failed`, variant: "destructive" });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => toast({ title: "Running health check on all scrapers..." })}
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-4">
              ...
              <div className="flex items-center gap-3 text-[10px]">
                <span className={`px-2 py-1 rounded ${scraper.userAgentRotation ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-700 text-slate-500'}`}>
                  <Shield size={10} className="inline mr-1" />UA
                </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  {Object.entries(scraper.selectors).map(([key, val]) => (
                    <div key={key} className="space-y-1">
                      ...
                      <input defaultValue={val} className="w-full bg-slate-800 border border-white/[0.08] rounded px-2 py-1.5 text-[11px] font-mono text-slate-300 focus:outline-none focus:ring-1 focus:ring-blue-500" />
                    </div>
                  ))}
                </div>
                <button onClick={() => { apiCall(`/scrapers/${scraper.id}`, 'PUT', scraper); toast({ title: "Selectors saved" }); }}
      ...
      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
        ...
        <p className="text-xs text-slate-500 mb-3">Manage proxy servers. Format: host:port:user:pass</p>
